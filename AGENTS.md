...
| `canonical_id_registry.csv` | **Frozen** registry of canonical IDs 1–404 (FY19–FY26 facilities). Append-only. New annual stats extend from 405; DMCP-sourced new facilities start at 1001. |
...
For a complete, auto-generated list of targets with descriptions and current dimensions, see [`data/targets-catalog.md`](data/targets-catalog.md). The catalog is regenerated by the `targets_catalog` target each pipeline run. Target groups in the catalog: Core annual stats · Geocoding · DMCP supplemental listings · DDP daily population · Unified DETLOC lookup · Hold facility and ERO integration · Marshall Project locations · Vera Institute facility metadata · Wikipedia harmonization · Source presence and outputs.

**Catalog gaps (as of 2026-05-24):** The following targets are present in `_targets.R` but absent from `data/targets-catalog.md`. To add them, extend `target_descriptions()` in `R/catalog.R` and run `tar_make(targets_catalog)`.

- **Geocoding helpers:** `facilities_geocoding_lookup`, `google_geocoded_file`, `facilities_geocoded_all_file`
- **Extended DETLOC:** `detloc_lookup_complete` — extends `detloc_lookup_full` with `ddp_facility_canonical` (IDs 1054–1209, 3001–3226); covers all 962 canonical IDs
- **DDP parquet / new data:** `ddp_parquet_file`, `ddp_new` (parquet, Oct 2022–Mar 2026, 707 facilities), `ddp_codes_new`
- **Expanded map pipeline:** `ddp_annual_panel`, `expanded_map_panel`, `expanded_map_presence`, `expanded_map_geocoded`, `expanded_map_export` (cue = "never")
- **Facility directory:** `facility_directory_export` (cue = "never")
- **Facility profiles:** `stints_file`, `stints_raw` (DDP detention stints parquet, 2.6M rows), `facility_profiles_data` (before/after 2025-01-20 profiles for 5 facility groups), `facility_profiles_report` (`tar_quarto()`)
- **FY26 comparison pipeline:** `fy26b`, `ddp_fy26_keyed`, `daily_totals_fy26`, `unmatched_fy26`, `peak_fy26`, `ddp_fy26_comparison_export` (cue = "never")
- **ICE office node scan:** `ice_office_nodes`, `ice_office_nodes_es` (cue = "never"; 110 ERO sub-office locations parsed from cached ice.gov node pages)
- **Hold facility registry:** `hold_canonical_registry_file`, `hold_canonical_registry`, `missing_hold_addresses_file`, `missing_hold_addresses`

Two catalog entries are also stale (renamed): `facilities_geocoded` and `facilities_geocoded_full` no longer exist — the current geocoding target is `facilities_google_geocoded`.
...
| Range | Purpose |
|----|----|
| 1–404 | FY19–FY26 facilities — **frozen** in `data/canonical_id_registry.csv` (IDs 1–398 original freeze; 399–404 added for new panel facilities) |
| 405–1000 | Reserved for future annual stats (FY27+), continuing sequentially |
| 1001–1053 | Facilities sourced from faclist15/faclist17 only (not in FY19–FY26 panel) |
| 1054–1209 | DDP non-hold, non-medical facilities (155: ~109 Non-Dedicated jails, ~28 Federal, ~9 Dedicated, ~3 Family/Youth, ~1 Other/Unknown) |
| 1210–2000 | Reserved for future supplemental detention facility sources |
...
| Canonical facilities (IDs 1–404) with a DETLOC | 308+ of 344 (via detloc_lookup); all 962 covered via detloc_lookup_complete |
...
Four posts are published in `~/Dropbox (Personal)/R/quarto-website/posts/`:

| Post directory | Pipeline export target | Description |
|---|---|---|
| `ddp-comparison/` | `ddp_comparison_export` | DDP (Sep 2023–Oct 2025, feather) vs. ICE FY25 annual statistics; unreported facility analysis. Exports 11 RDS files to `data/ddp-comparison-export/`. |
| `ddp-comparison-26/` | `ddp_fy26_comparison_export` | DDP (Oct 2022–Mar 2026, parquet) vs. ICE FY26 annual statistics; comparison period Oct 2025–Feb 5 2026. Exports 8 RDS files to `data/ddp-comparison-export-fy26/`. |
| `ice-detention-map/` | *(none — rendered from data/ CSVs directly)* | Interactive Leaflet map of ~390 panel facilities (FY19–FY26 ICE annual stats only). |
| `ice-detention-map-expanded/` | `expanded_map_export` | Expanded Leaflet map including DDP-only facilities (~960+ facilities across all canonical ID ranges). Exports `expanded_panel.rds`, `expanded_presence.rds`, `expanded_geocoded.rds` to `data/expanded-map-export/`. |
| `facility-directory/` | `facility_directory_export` | Reactable facility directory table with ADP bar charts and DDP sparklines for ~618 facilities (peak ≥ 2). Exports `facility_tbl.rds` to `data/facility-directory-export/`. |

All export targets are `cue = "never"` — run `tar_make(<export_target>)` explicitly to update. The general deploy workflow for each post:

1. `tar_make(<export_target>)` — write/update pre-computed RDS files in the export directory.
2. For DDP comparison posts, review the local Quarto render first (`ddp_comparison_report` target).
3. Run `copy-data.sh` in the post directory to copy files to `posts/<post>/data/`.
